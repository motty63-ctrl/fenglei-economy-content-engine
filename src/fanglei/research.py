...
def deduplicate_sources(documents: list[FetchedDocument]) -> list[dict[str, Any]]:
    ...
    for doc in documents:
        ...
        institution = urlsplit(origin).netloc.lower()
        ...
        authoritative_key = f"institution:{institution}" if is_authoritative_origin and not doc.original_url else None
...
class RuleBasedEvidenceExtractor:
    _markers = re.compile(r"(?:\d[\d,.]*\s*%?|GDP|国内生产总值|通胀|利率|就业|汇率|政策)", re.I)
        ...
        for doc in documents:
            ...
            for index, paragraph in enumerate(paragraphs, 1):
                for sentence in re.split(r"(?<=[。！？!?])|(?<!\d)\.(?!\d)", paragraph):
                    ...
                    if sentence and self._markers.search(sentence):
...
def verify_claims(
    ...
    *,
    ...
    for item in evidence:
        ...
        key = _claim_identity(compact)[0] if item.get("claim_type") == "fact" else compact
    ...
    for index, items in enumerate(groups.values(), 1):
        ...
        values = {_claim_identity(item["evidence_text"])[1] for item in items}
        ...
        clean_evidence = [{key: value for key, value in item.items() if key != "claim_type"} for item in items]
        claims.append(
            {
                "claim_id": f"claim_{index:03d}",
                "claim_text": items[0]["evidence_text"],
                "claim_type": claim_type,
