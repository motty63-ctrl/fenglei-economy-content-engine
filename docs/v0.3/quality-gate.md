...
`script.md` is written and marked valid only when every hard gate passes.
...
7. **Duration:** spoken content is 240–360 effective Chinese characters at 4.0 characters/second, yielding 60–90 seconds. Metadata, Markdown, and claim markers are excluded.
...
12. **Traceability:** every visible `〔claim:claim_NNN〕` marker resolves; every verified-fact sentence has a marker; non-factual sentences do not imply external attribution.
